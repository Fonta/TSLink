		...
		$PL->settings('tslink_settings', $lang->setting_group_tslink, $lang->setting_group_tslink_desc, 
			array(
	 		...
	 		else 
	 		{
				...
				$db->query("ALTER TABLE " . TABLE_PREFIX . "users ADD (`memberstatus` int(1) NOT NULL DEFAULT 0)");	
	 		}
...
<td class="{$alt_bg}" align="center">{$linktochange}</td>
	...
	if ($mybb->settings['tslink_settings_enabled']) {
		...
		if ($mybb->settings['tslink_settings_onregister']) {
			...
			$plugins->add_hook("member_activate_accountactivated", "tslink_doupdategroups", $givenip);
			$plugins->add_hook("fb_register_end", "tslink_doupdategroups", $givenip);
		}
