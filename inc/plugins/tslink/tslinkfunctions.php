<?php
			...
			if (!$mybb->input['action'])
            {
				...
				while($users = $db->fetch_array($query))
                {
					...
					if ($users['memberstatus'] == '0')
						{ 
							$form_container->output_cell("<div style=\"\"><strong>Member</strong></div>", array("class" => "align_center"));
							$form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto=1\">{$lang->tslink_admin_row_changestatus1}</a>", array("class" => "align_center"));
							$form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto=2\">{$lang->tslink_admin_row_changestatus2}</a>", array("class" => "align_center"));
						}
					elseif ($users['memberstatus'] == '1')
						{ 
							$form_container->output_cell("<div style=\"\"><strong>Donating Member</strong></div>", array("class" => "align_center"));
							$form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto=0\">{$lang->tslink_admin_row_changestatus0}</a>", array("class" => "align_center"));
							$form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto=2\">{$lang->tslink_admin_row_changestatus2}</a>", array("class" => "align_center"));
						}
					elseif ($users['memberstatus'] == '2')
						{ 
							$form_container->output_cell("<div style=\"\"><strong>VIP Member</strong></div>", array("class" => "align_center"));
							$form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto=0\">{$lang->tslink_admin_row_changestatus0}</a>", array("class" => "align_center"));
							$form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto=1\">{$lang->tslink_admin_row_changestatus1}</a>", array("class" => "align_center"));
						}
    ...
    function tslink_update_groups($mybb_uid)
	{
        ...
        foreach ($ts_unique_ids as $ts_unique_id) {
            ...
            $ClientServerGroups=$ts3_VirtualServer->execute("servergroupsbyclientid", array("cldbid" => $ts_unique_id['ts_cldbid']))->toAssocArray("sgid");
            $c=0;
            ...
            {                
                    ...
                    foreach ($csg as $ClientServerGroupID)
                    {
                        ...
                        else
                        {
                            try 
                            {
                                $ts3_VirtualServer->execute("servergroupdelclient", array("sgid" => $ClientServerGroupID, "cldbid" => $ts_unique_id["ts_cldbid"]));
                            }
                            catch (Exception $e)
                            {
                                // Catches the error(s) if any. But don't do anything with it.
            ...
            try 
            {
                ...
                $ts3_VirtualServer->execute("servergroupaddclient", array("sgid" => $ServerGroupID_ToAdd, "cldbid" => $ts_unique_id["ts_cldbid"]));
            }
            catch (Exception $e) {
                // Catches the error(s) if any. But don't do anything with it.
            }
