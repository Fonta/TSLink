        ...
        if ($run_module == 'user' && $action_file == 'tslink') {
            ...
            if (!$mybb->input['action']) {
                ...
                while ($users = $db->fetch_array($query)) {
                    ...
                    $form_container->output_cell("<div style=\"\"><strong>{$status}</strong></div>", ['class' => 'align_center']);
                    $form_container->output_cell("<a href=\"index.php?module=user-tslink&amp;action=tslink_changestatus&amp;uid={$users['uid']}&amp;changeto={$statustochangeto}\">{$lang->tslink_admin_row_changestatus}</a>", ['class' => 'align_center']);
