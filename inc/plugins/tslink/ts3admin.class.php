...
//*******************************************************************************************
     ...
     */
    private $runtime = ['socket' => '', 'selected' => false, 'host' => '', 'queryport' => '10011', 'timeout' => 2, 'debug' => [], 'fileSocket' => ''];
...
//******************************************************************************************
     ...
     * @param int    $time      bantime in seconds (0=unlimited)
     ...
     * @return bool success
     */
    public function ftCreateDir($cid, $cpw, $dirname)
    {
        if (!$this->runtime['selected']) {
            return $this->checkSelected();
        }
     ...
     * @return bool success
     */
    public function ftRenameFile($cid, $cpw, $oldname, $newname, $tcid = null, $tcpw = null)
    {
        if (!$this->runtime['selected']) {
            return $this->checkSelected();
        }
    ...
    {
        $this->logout();
        @fwrite($this->runtime['socket'], "quit\n");
...
//*******************************************************************************************
    ...
    private function executeCommand($command, $tracert)
    {
        ...
        foreach ($splittedCommand as $commandPart) {
            fwrite($this->runtime['socket'], $commandPart);
        }
    ...
    {
        fwrite($this->runtime['fileSocket'], $key.$additional);
    }
    ...
    private function ftSendData($data)
    {
        ...
        foreach ($data as $dat) {
            fwrite($this->runtime['fileSocket'], $dat);
        }
    }
